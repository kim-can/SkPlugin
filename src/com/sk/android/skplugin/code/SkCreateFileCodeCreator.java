package com.sk.android.skplugin.code;
 ...
 */
public class SkCreateFileCodeCreator extends WriteCommandAction.Simple {
	...
	protected static final Logger	log	= Logger.getInstance(SkCreateFileCodeCreator.class);

	public SkCreateFileCodeCreator(Project project, PsiDirectory psiDirectory, int selectType, String className, String extendsName, int extendsIndex, String command) {
	...
	private void runView(String className) throws IOException {
		...
		switch (extendsIndex) {
			case 0: // activity
				...
				view = generateView(viewName, xmlName, bizName, "sk.SKActivity");
				break;
			case 1: // fragment
				...
				gennerateXml(xmlName,packageName + "." + viewName);
				view = generateView(viewName, xmlName, bizName, "sk.SKFragment");
				break;
			case 2: // dialog fragment
				if (!viewName.endsWith("DialogFragment")) {
					viewName = viewName + "DialogFragment";
				}
				xmlName = "dialog_fragment_" + xmlName.toLowerCase();

				gennerateXml(xmlName,packageName + "." + viewName);
				view = generateView(viewName, xmlName, bizName, "sk.SKDialogFragment");
				break;
		}
	...
	private PsiFile generateAdapterMore(String viewName, String xmlTopName, String xmlBottomName) throws IOException {
		...
		FileTemplate template = FileTemplateManager.getInstance(mProject).getInternalTemplate("SkAdapterMoreTemplate");
	...
	private PsiFile generateAdapter(String viewName, String xmlName) throws IOException {
		...
		FileTemplate template = FileTemplateManager.getInstance(mProject).getInternalTemplate("SkAdapterTemplate");
		Properties defaultProperties = FileTemplateManager.getInstance(mProject).getDefaultProperties();
		Properties properties = new Properties(defaultProperties);
		properties.setProperty("NAME", fileName.toString());
	...
	private PsiFile generateView(String viewName, String xmlName, String bizName, String superName) throws IOException {
		...
		if (superName.endsWith("SKActivity")) {
			importText.append("import sk.SKHelper;\n");
		}
		...
		switch (extendsIndex){
			case 0:
				importText.append("import sk.SKActivityBuilder;\n");

				builder = "SKActivityBuilder";
				break;
			case 1:
				importText.append("import sk.SKFragmentBuilder;\n");
				builder = "SKFragmentBuilder";
				break;
			case 2:
				importText.append("import sk.SKDialogFragmentBuilder;\n");
				builder = "SKDialogFragmentBuilder";
				break;
		}
		...
		if (superName.endsWith("SKActivity")) {
			...
			methodText.append("\t\tSKHelper.display().intent(" + viewName + ".class);\n");
			methodText.append("\t}");
		} else {
		...
		if (superName.endsWith("SKDialogFragment")) {
			// ADD method
			methodDialogText.append("\t\n@Override protected int style() {\n" + "        return 0;\n" + "    }\n");
		}

		FileTemplate template = FileTemplateManager.getInstance(mProject).getInternalTemplate("SkViewTemplate");
		Properties defaultProperties = FileTemplateManager.getInstance(mProject).getDefaultProperties();
		Properties properties = new Properties(defaultProperties);
		properties.setProperty("NAME", fileName.toString());
		...
		properties.setProperty("BUILDER",builder);
	...
	private PsiFile generateBiz(String bizName, String viewName) throws IOException {
		...
		FileTemplate template = FileTemplateManager.getInstance(mProject).getInternalTemplate("SkBizTemplate");
		Properties defaultProperties = FileTemplateManager.getInstance(mProject).getDefaultProperties();
		Properties properties = new Properties(defaultProperties);
		properties.setProperty("NAME", fileName.toString());
